...
<template>
  <div class="bg-slate-50 font-sans">
    ...
    <main class="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
      ...
      <!-- Destinations Grid (2-Kolom di Mobile, 3-Kolom di Desktop) -->
      <div v-if="filteredDestinations.length > 0" class="grid grid-cols-2 lg:grid-cols-3 gap-2.5 sm:gap-6">
        <article
          v-for="(dest, idx) in filteredDestinations"
          :key="dest.id"
          v-reveal:fade-up="(idx % 3) * 90"
          class="bg-white rounded-xl sm:rounded-2xl border border-slate-200/90 overflow-hidden shadow-xs hover:shadow-md hover:border-slate-300 transition-all flex flex-col group"
        >
          ...
          <div class="relative aspect-[4/3] sm:aspect-[16/10] overflow-hidden bg-slate-900">
            ...
            <div class="absolute top-2 left-2 sm:top-3 sm:left-3 z-10">
              <span class="inline-flex items-center px-1.5 sm:px-2.5 py-0.5 sm:py-1 rounded sm:rounded-md text-[9px] sm:text-[11px] font-bold bg-slate-900/85 text-slate-100 border border-white/20 backdrop-blur-sm shadow-xs">
                {{ isEnglish ? (dest.badgeEn || dest.badge) : dest.badge }}
              </span>
            </div>

            <!-- Official Photo Credit Badge (Hidden on mobile to keep 2-column photo clean) -->
            <div class="hidden sm:block absolute top-3 right-3 z-10">
              <span
            ...
            <div class="absolute bottom-2 left-2.5 right-2.5 sm:bottom-3 sm:left-4 sm:right-4 z-10">
              <h2 class="text-xs sm:text-base md:text-lg font-bold text-white leading-snug drop-shadow-md group-hover:text-blue-200 transition-colors line-clamp-1">
                {{ isEnglish ? (dest.nameEn || dest.name) : dest.name }}
              </h2>
              <p class="text-[10px] sm:text-xs text-slate-300 flex items-center gap-1 mt-0.5 sm:mt-1">
                <svg class="w-3 h-3 sm:w-3.5 sm:h-3.5 text-blue-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div class="p-2.5 sm:p-5 flex-1 flex flex-col justify-between space-y-2.5 sm:space-y-4">
            <!-- Description -->
            <p class="text-[11px] sm:text-sm text-slate-600 leading-relaxed line-clamp-2 sm:line-clamp-none">
              {{ isEnglish ? (dest.descriptionEn || dest.description) : dest.description }}
            </p>

            <!-- Key Highlights List -->
            <div class="space-y-1 sm:space-y-1.5 bg-slate-50 p-2 sm:p-3 rounded-lg sm:rounded-xl border border-slate-100 text-[10px] sm:text-xs text-slate-700">
              <div class="text-[9px] sm:text-[11px] font-bold text-slate-900 uppercase tracking-wider mb-0.5 sm:mb-1 flex items-center gap-1 sm:gap-1.5">
                <span class="w-1 h-1 rounded-full bg-blue-600"></span>
                {{ isEnglish ? 'Highlights:' : 'Daya Tarik:' }}
              </div>
              <ul class="space-y-0.5 sm:space-y-1">
                <li
                  v-for="(highlight, hIdx) in (isEnglish ? (dest.highlightsEn || dest.highlights) : dest.highlights)"
                  :key="hIdx"
                  class="flex items-start gap-1 sm:gap-1.5 text-[10px] sm:text-[11px] leading-snug"
                  :class="{ 'hidden sm:flex': hIdx >= 2 }"
                >
                  <span class="text-blue-600 font-bold shrink-0">•</span>
                  <span class="line-clamp-1 sm:line-clamp-none">{{ highlight }}</span>
                </li>
              </ul>
            </div>

            <!-- Route & Recommended Fleet Info -->
            <div class="pt-1.5 sm:pt-2 border-t border-slate-100 space-y-1 sm:space-y-1.5 text-[10px] sm:text-xs">
              <div class="flex items-center gap-1.5 sm:gap-2 text-slate-500 text-[10px] sm:text-[11px]">
                ...
                <span class="truncate">{{ isEnglish ? (dest.driveTimeEn || dest.driveTime) : dest.driveTime }}</span>
              </div>
              <div class="flex items-center gap-1.5 sm:gap-2 text-slate-500 text-[10px] sm:text-[11px]">
                <svg class="w-3 h-3 sm:w-3.5 sm:h-3.5 text-slate-400 shrink-0" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M18.92 6.01C18.72 5.42 18.16 5 17.5 5h-11c-.66 0-1.21.42-1.42 1.01L3 12v8c0 .55.45 1 1 1h1c.55 0 1-.45 1-1v-1h12v1c0 .55.45 1 1 1h1c.55 0 1-.45 1-1v-8l-2.08-5.99zM6.5 16c-.83 0-1.5-.67-1.5-1.5S5.67 13 6.5 13s1.5.67 1.5 1.5S7.33 16 6.5 16zm11 0c-.83 0-1.5-.67-1.5-1.5s.67-1.5 1.5-1.5 1.5.67 1.5 1.5-.67 1.5-1.5 1.5zM5 11l1.5-4.5h11L19 11H5z"/>
                </svg>
                <span class="truncate">{{ isEnglish ? 'Fleet:' : 'Armada:' }} <strong class="text-slate-700">{{ isEnglish ? (dest.recommendedFleetEn || dest.recommendedFleet) : dest.recommendedFleet }}</strong></span>
              </div>
            </div>

            <!-- Action CTA Buttons -->
            <div class="pt-1.5 sm:pt-2 flex flex-col sm:flex-row items-stretch sm:items-center gap-1.5 sm:gap-2">
              <a
                :href="getDestWhatsAppUrl(dest)"
                target="_blank"
                rel="noopener noreferrer"
                class="w-full sm:flex-1 py-2 sm:py-2.5 px-2 sm:px-3 rounded-lg sm:rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-[11px] sm:text-xs font-bold inline-flex items-center justify-center gap-1 sm:gap-1.5 shadow-xs transition-all active:scale-95 text-center"
              >
                ...
                <span>{{ isEnglish ? 'Rent Car' : 'Sewa Mobil' }}</span>
              </a>

              <RouterLink
                to="/paket-tour-bintan"
                class="w-full sm:w-auto py-1.5 sm:py-2.5 px-2 sm:px-3 rounded-lg sm:rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 text-[10px] sm:text-xs font-semibold inline-flex items-center justify-center transition-colors text-center shrink-0"
