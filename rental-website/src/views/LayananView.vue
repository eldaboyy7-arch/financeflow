<script setup lang="ts">
import { RouterLink } from 'vue-router'
import { siteConfig } from '@/config/site'

const waUrl = (text: string) => {
  const clean = siteConfig.rentalPhone.replace(/\D/g, '')
  return `https://wa.me/${clean}?text=${encodeURIComponent(text)}`
}

const whyUs = [
  { icon: '🚗', title: 'Armada Terawat', desc: 'Unit selalu bersih, ber-AC dingin, dan diperiksa sebelum perjalanan.' },
  { icon: '👨‍✈️', title: 'Driver Berpengalaman', desc: 'Hafal seluruh sudut Bintan & Tanjung Pinang, ramah dan tepat waktu.' },
  { icon: '💰', title: 'Harga Transparan', desc: 'Tidak ada biaya tersembunyi. Semua sudah jelas di awal sebelum booking.' },
  { icon: '⚡', title: 'Respon Cepat', desc: 'Chat WhatsApp langsung dibalas — rata-rata dalam hitungan menit.' },
]
</script>

<template>
  <div class="min-h-screen bg-white font-sans">

    <!-- ══════════════════════════════════════
         HERO
    ══════════════════════════════════════ -->
    <section class="relative min-h-[480px] sm:min-h-[560px] flex items-end overflow-hidden">
      <img
        src="/images/layanan-hero.jpg"
        alt="Layanan Rental Mobil Bintan"
        class="absolute inset-0 w-full h-full object-cover object-center"
      />
      <!-- Gradient: gelap bawah untuk teks, subtle atas -->
      <div class="absolute inset-0 bg-gradient-to-t from-slate-950 via-slate-950/50 to-slate-900/10 pointer-events-none"></div>

      <div class="relative z-10 w-full max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pb-12 sm:pb-16">
        <p class="text-xs font-bold uppercase tracking-[0.15em] text-amber-400 mb-3">
          3 Putri Mulya · Bintan & Tanjung Pinang
        </p>
        <h1 class="text-3xl sm:text-5xl font-black text-white tracking-tight leading-tight mb-3 max-w-lg">
          Layanan untuk Setiap Perjalanan Anda
        </h1>
        <p class="text-sm sm:text-base text-white/60 mb-7 max-w-sm">
          Rental Mobil &nbsp;·&nbsp; Driver &nbsp;·&nbsp; Tour &nbsp;·&nbsp; Antar-Jemput
        </p>
        <a
          :href="waUrl('Halo 3 Putri Mulya, saya ingin tanya informasi layanan yang tersedia.')"
          target="_blank"
          rel="noopener noreferrer"
          class="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-emerald-500 hover:bg-emerald-400 text-white font-bold text-sm transition-all active:scale-95"
        >
          <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
            <path d="M12.031 6.172c-3.181 0-5.767 2.586-5.768 5.766-.001 1.298.38 2.27 1.019 3.287l-.582 2.128 2.182-.573c.978.58 1.911.928 3.145.929 3.178 0 5.767-2.587 5.768-5.766.001-3.187-2.575-5.771-5.764-5.771zm3.392 8.244c-.144.405-.837.774-1.17.824-.299.045-.677.063-1.092-.069-.252-.08-.575-.187-.988-.365-1.739-.751-2.874-2.502-2.961-2.617-.087-.116-.708-.94-.708-1.793s.448-1.273.607-1.446c.159-.173.346-.217.462-.217l.332.006c.106.005.249-.04.39.298.144.347.491 1.2.534 1.287.043.087.072.188.014.304-.058.116-.087.188-.173.289l-.26.304c-.087.086-.177.18-.076.354.101.174.449.741.964 1.201.662.591 1.221.774 1.394.86.174.086.275.072.376-.043.101-.116.433-.506.549-.68.116-.173.231-.145.39-.087s1.011.477 1.184.564.289.13.332.202c.043.073.043.419-.101.824z"/>
          </svg>
          Konsultasi Gratis
        </a>
      </div>
    </section>

    <!-- ══════════════════════════════════════
         2 LAYANAN UTAMA
    ══════════════════════════════════════ -->
    <section class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pt-12 sm:pt-16 pb-6">
      <p class="text-[11px] font-bold uppercase tracking-widest text-slate-400 mb-6">Layanan Utama</p>

      <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">

        <!-- Card 1: Lepas Kunci -->
        <div class="group relative rounded-2xl overflow-hidden bg-slate-900 min-h-[320px] sm:min-h-[380px] flex flex-col justify-end">
          <img
            src="/images/layanan-hero.jpg"
            alt="Sewa Lepas Kunci Bintan"
            class="absolute inset-0 w-full h-full object-cover object-center group-hover:scale-105 transition-transform duration-500"
          />
          <div class="absolute inset-0 bg-gradient-to-t from-slate-950/95 via-slate-950/40 to-transparent"></div>
          <div class="relative z-10 p-6">
            <span class="inline-block text-[10px] font-bold uppercase tracking-widest text-amber-400 mb-2">Paling Fleksibel</span>
            <h2 class="text-xl font-black text-white mb-1.5">Sewa Lepas Kunci</h2>
            <p class="text-xs text-white/65 leading-relaxed mb-5">
              Pesan mobil, ambil kunci, bebas kemana saja. City Car, MPV, hingga HiAce — harga harian transparan.
            </p>
            <div class="flex items-center gap-3">
              <RouterLink
                to="/armada"
                class="inline-flex items-center gap-1.5 px-4 py-2 rounded-full bg-white text-slate-900 font-bold text-xs hover:bg-slate-100 transition-colors"
              >
                Lihat Armada
                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 5l7 7-7 7"/>
                </svg>
              </RouterLink>
              <a
                :href="waUrl('Halo 3 Putri Mulya, saya ingin sewa mobil lepas kunci di Bintan.')"
                target="_blank" rel="noopener noreferrer"
                class="text-xs font-bold text-white/70 hover:text-white transition-colors"
              >Tanya Admin</a>
            </div>
          </div>
        </div>

        <!-- Card 2: Dengan Supir -->
        <div class="group relative rounded-2xl overflow-hidden bg-slate-900 min-h-[320px] sm:min-h-[380px] flex flex-col justify-end">
          <img
            src="/images/layanan-driver.jpg"
            alt="Sewa Mobil dengan Supir Bintan"
            class="absolute inset-0 w-full h-full object-cover object-center group-hover:scale-105 transition-transform duration-500"
          />
          <div class="absolute inset-0 bg-gradient-to-t from-slate-950/95 via-slate-950/40 to-transparent"></div>
          <div class="relative z-10 p-6">
            <span class="inline-block text-[10px] font-bold uppercase tracking-widest text-emerald-400 mb-2">Paling Santai</span>
            <h2 class="text-xl font-black text-white mb-1.5">Sewa dengan Supir</h2>
            <p class="text-xs text-white/65 leading-relaxed mb-5">
              Duduk santai, nikmati perjalanan. Driver berpengalaman siap mengantar rombongan seharian penuh.
            </p>
            <div class="flex items-center gap-3">
              <RouterLink
                to="/paket-tour-bintan"
                class="inline-flex items-center gap-1.5 px-4 py-2 rounded-full bg-white text-slate-900 font-bold text-xs hover:bg-slate-100 transition-colors"
              >
                Lihat Paket
                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 5l7 7-7 7"/>
                </svg>
              </RouterLink>
              <a
                :href="waUrl('Halo 3 Putri Mulya, saya ingin sewa mobil dengan supir untuk perjalanan di Bintan.')"
                target="_blank" rel="noopener noreferrer"
                class="text-xs font-bold text-white/70 hover:text-white transition-colors"
              >Tanya Admin</a>
            </div>
          </div>
        </div>

      </div>
    </section>

    <!-- ══════════════════════════════════════
         PAKET TOUR — Full Width
    ══════════════════════════════════════ -->
    <section class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      <div class="group relative rounded-2xl overflow-hidden bg-slate-900 min-h-[260px] sm:min-h-[300px] flex items-end">
        <img
          src="/images/destinations/lagoi.jpg"
          alt="Paket Tour Bintan"
          class="absolute inset-0 w-full h-full object-cover object-center group-hover:scale-105 transition-transform duration-500"
        />
        <div class="absolute inset-0 bg-gradient-to-r from-slate-950/95 via-slate-950/60 to-slate-950/10 pointer-events-none"></div>
        <div class="relative z-10 p-6 sm:p-8 max-w-lg">
          <span class="inline-block text-[10px] font-bold uppercase tracking-widest text-amber-400 mb-2">All-In</span>
          <h2 class="text-xl sm:text-2xl font-black text-white mb-2">Paket Tour Bintan</h2>
          <p class="text-xs sm:text-sm text-white/65 leading-relaxed mb-5">
            Keliling destinasi ikonik dalam satu hari. HiAce + Supir + BBM sudah include. Sleeping Buddha, Lagoi, Gonggong, Patung Seribu — tinggal datang dan nikmati.
          </p>
          <RouterLink
            to="/paket-tour-bintan"
            class="inline-flex items-center gap-1.5 px-5 py-2.5 rounded-full bg-amber-400 hover:bg-amber-300 text-slate-900 font-bold text-xs transition-colors"
          >
            Lihat Paket Tour
            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 5l7 7-7 7"/>
            </svg>
          </RouterLink>
        </div>
      </div>
    </section>

    <!-- ══════════════════════════════════════
         ANTAR JEMPUT — Banner Tipis
    ══════════════════════════════════════ -->
    <section class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-5 border border-slate-200 rounded-2xl px-6 py-6">
        <div>
          <span class="text-[10px] font-bold uppercase tracking-widest text-violet-500 block mb-1.5">Tepat Waktu</span>
          <h2 class="text-lg font-black text-slate-900 mb-1">Antar Jemput Bandara & Ferry</h2>
          <p class="text-sm text-slate-500 max-w-md">
            Bandara RHF &nbsp;·&nbsp; Pelabuhan Sri Bintan Pura &nbsp;·&nbsp; Untuk personal hingga rombongan besar. Pesan H-1 atau lebih awal.
          </p>
        </div>
        <a
          :href="waUrl('Halo 3 Putri Mulya, saya ingin pesan layanan antar/jemput bandara atau pelabuhan di Bintan.')"
          target="_blank" rel="noopener noreferrer"
          class="shrink-0 inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-slate-900 hover:bg-slate-700 text-white font-bold text-sm transition-all whitespace-nowrap"
        >
          <svg class="w-4 h-4 text-emerald-400" fill="currentColor" viewBox="0 0 24 24">
            <path d="M12.031 6.172c-3.181 0-5.767 2.586-5.768 5.766-.001 1.298.38 2.27 1.019 3.287l-.582 2.128 2.182-.573c.978.58 1.911.928 3.145.929 3.178 0 5.767-2.587 5.768-5.766.001-3.187-2.575-5.771-5.764-5.771zm3.392 8.244c-.144.405-.837.774-1.17.824-.299.045-.677.063-1.092-.069-.252-.08-.575-.187-.988-.365-1.739-.751-2.874-2.502-2.961-2.617-.087-.116-.708-.94-.708-1.793s.448-1.273.607-1.446c.159-.173.346-.217.462-.217l.332.006c.106.005.249-.04.39.298.144.347.491 1.2.534 1.287.043.087.072.188.014.304-.058.116-.087.188-.173.289l-.26.304c-.087.086-.177.18-.076.354.101.174.449.741.964 1.201.662.591 1.221.774 1.394.86.174.086.275.072.376-.043.101-.116.433-.506.549-.68.116-.173.231-.145.39-.087s1.011.477 1.184.564.289.13.332.202c.043.073.043.419-.101.824z"/>
          </svg>
          Tanya Admin WA
        </a>
      </div>
    </section>

    <!-- ══════════════════════════════════════
         KENAPA 3 PUTRI MULYA
    ══════════════════════════════════════ -->
    <section class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-14">
      <p class="text-[11px] font-bold uppercase tracking-widest text-slate-400 mb-8">Kenapa Memilih Kami?</p>
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-5 sm:gap-6">
        <div v-for="w in whyUs" :key="w.title" class="space-y-2">
          <div class="text-2xl">{{ w.icon }}</div>
          <h3 class="text-sm font-black text-slate-900">{{ w.title }}</h3>
          <p class="text-xs text-slate-500 leading-relaxed">{{ w.desc }}</p>
        </div>
      </div>
    </section>

    <!-- ══════════════════════════════════════
         BOTTOM CTA
    ══════════════════════════════════════ -->
    <section class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
      <div class="bg-slate-900 rounded-2xl px-7 sm:px-10 py-9 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-5">
        <div>
          <p class="text-[11px] font-bold uppercase tracking-widest text-slate-500 mb-1.5">Butuh Saran?</p>
          <h3 class="text-lg sm:text-xl font-black text-white">Tidak yakin pilih layanan mana?</h3>
          <p class="text-sm text-slate-400 mt-1">Konsultasi gratis, kami bantu sesuaikan kebutuhan dan budget kamu.</p>
        </div>
        <a
          :href="waUrl('Halo 3 Putri Mulya, saya ingin konsultasi untuk memilih layanan sewa yang sesuai dengan kebutuhan perjalanan saya di Bintan.')"
          target="_blank" rel="noopener noreferrer"
          class="shrink-0 inline-flex items-center gap-2 px-6 py-3 rounded-full bg-emerald-500 hover:bg-emerald-400 text-white font-bold text-sm transition-all active:scale-95 whitespace-nowrap"
        >
          <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
            <path d="M12.031 6.172c-3.181 0-5.767 2.586-5.768 5.766-.001 1.298.38 2.27 1.019 3.287l-.582 2.128 2.182-.573c.978.58 1.911.928 3.145.929 3.178 0 5.767-2.587 5.768-5.766.001-3.187-2.575-5.771-5.764-5.771zm3.392 8.244c-.144.405-.837.774-1.17.824-.299.045-.677.063-1.092-.069-.252-.08-.575-.187-.988-.365-1.739-.751-2.874-2.502-2.961-2.617-.087-.116-.708-.94-.708-1.793s.448-1.273.607-1.446c.159-.173.346-.217.462-.217l.332.006c.106.005.249-.04.39.298.144.347.491 1.2.534 1.287.043.087.072.188.014.304-.058.116-.087.188-.173.289l-.26.304c-.087.086-.177.18-.076.354.101.174.449.741.964 1.201.662.591 1.221.774 1.394.86.174.086.275.072.376-.043.101-.116.433-.506.549-.68.116-.173.231-.145.39-.087s1.011.477 1.184.564.289.13.332.202c.043.073.043.419-.101.824z"/>
          </svg>
          Chat WhatsApp Sekarang
        </a>
      </div>
    </section>

  </div>
</template>
