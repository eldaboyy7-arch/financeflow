...
function getCountdownBadge(event: BintanEvent) {
  ...
  if (status === 'live') {
    return {
      ...
      classes: 'bg-rose-600 text-white animate-pulse font-extrabold',
      icon: '🔴'
    }
  }
  ...
  if (days <= 0) {
    return {
      text: isEnglish.value ? 'Starts Today!' : 'Dimulai Hari Ini!',
      classes: 'bg-amber-500 text-white font-extrabold',
      icon: '⚡'
    }
  }
  if (days <= 30) {
    return {
      text: isEnglish.value ? `H-${days} Days Left` : `H-${days} Hari Lagi`,
      classes: 'bg-gradient-to-r from-orange-500 to-amber-500 text-white font-black shadow-xs',
      icon: '🔥'
    }
  }
  if (days <= 60) {
    return {
      text: isEnglish.value ? `H-${days} Days` : `H-${days} Hari`,
      classes: 'bg-indigo-600 text-white font-bold shadow-xs',
      icon: '📅'
    }
  }
  return {
    text: isEnglish.value ? `H-${days} Days` : `H-${days} Hari`,
    classes: 'bg-slate-800/90 text-white font-semibold',
    icon: '🗓️'
  }
}
  ...
  <section id="upcoming-events" class="py-12 sm:py-16 bg-slate-50 border-t border-slate-200/80 scroll-mt-16 sm:scroll-mt-20">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div v-reveal:fade-up class="flex flex-col sm:flex-row sm:items-end justify-between gap-3 mb-6 sm:mb-10">
        <div>
          <div class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-amber-100 text-amber-800 text-[10px] sm:text-xs font-black uppercase tracking-wider mb-2">
            <span>✨</span>
          ...
          >
            ...
            <div class="relative aspect-[16/10] bg-slate-900 overflow-hidden">
              ...
              <div class="absolute top-2.5 left-2.5 right-2.5 flex items-center justify-between gap-1.5 pointer-events-none">
                ...
                <span :class="['inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px]', getCountdownBadge(event).classes]">
                  <span>{{ getCountdownBadge(event).icon }}</span>
            ...
            <div class="p-4 flex-1 flex flex-col justify-between">
              ...
              <div>
                <!-- Recommended Fleet Pill -->
                <div class="p-2 rounded-lg bg-indigo-50/70 border border-indigo-100/80 mb-3 text-[11px] text-indigo-950 flex items-start gap-1.5">
                  <span class="text-sm leading-none shrink-0">🚐</span>
                  <div class="leading-tight">
                    <span class="font-bold block text-indigo-900">{{ t('events.recommendedFleet') }}</span>
                    <span class="text-indigo-700">{{ isEnglish ? event.recommendedFleetEn : event.recommendedFleet }}</span>
                  </div>
                </div>
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  class="w-full inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold shadow-xs transition-colors"
                >
      ...
      <div class="hidden sm:grid sm:grid-cols-2 lg:grid-cols-4 gap-5 lg:gap-6">
        ...
        >
          ...
          <div class="p-4 sm:p-5 pt-0">
            <!-- Recommended Fleet Pill -->
            <div class="p-2.5 rounded-xl bg-indigo-50/70 border border-indigo-100/80 mb-3 text-[11px] text-indigo-950 flex items-start gap-2">
              <span class="text-sm leading-none shrink-0 mt-0.5">🚐</span>
              <div class="leading-tight">
                <span class="font-bold block text-indigo-900">{{ t('events.recommendedFleet') }}</span>
                <span class="text-indigo-700 line-clamp-2">{{ isEnglish ? event.recommendedFleetEn : event.recommendedFleet }}</span>
              </div>
            </div>
